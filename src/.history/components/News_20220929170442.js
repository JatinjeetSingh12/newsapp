...
export default function News(props) {
    ...
    return (
        <>
            ...
            <div style={{ backgroundColor: props.mode === "light" ? "white" : "white", color: props.mode === "dark" ? "white" : "black" }}>
                        ...
                        <div className="row">
                            {!state.loading && article.map((element) => {
                                return <div className="col-md-4" key={element.url}>
                                    <Item title={element.title} mode={props.mode} source={element.source.name} author={element.author} date={element.publishedAt} description={element.description} imgUrl={element.urlToImage} Url={element.url} />
                                </div>
                            })}
